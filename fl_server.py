...
batch_size = 8 # Must be even, hsa to be split into 2 types of samples
...
four_files = [file for file in os.listdir("datasets_mine/train_json_four") if file.startswith("four")]
five_files = [file for file in os.listdir("datasets_mine/train_json_four") if file.startswith("five")]
six_files = [file for file in os.listdir("datasets_mine/train_json_four") if file.startswith("six")]
test_four_files = [file for file in os.listdir("datasets_mine/test_json_four") if file.startswith("four")]
test_five_files = [file for file in os.listdir("datasets_mine/test_json_four") if file.startswith("five")]
test_six_files = [file for file in os.listdir("datasets_mine/test_json_four") if file.startswith("six")]
...
random.shuffle(four_files)
random.shuffle(five_files)
random.shuffle(six_files)
...
numbers = list(sum(zip(one_files, two_files, three_files, unknown_files), ()))
test_numbers = list(sum(zip(test_one_files, test_two_files, test_three_files, test_unknown_files), ()))

# numbers = list(sum(zip(four_files, five_files, six_files), ()))
# test_numbers = list(sum(zip(test_four_files, test_five_files, test_six_files), ()))
...
def sendSamplesIID(device, deviceIndex, batch_size, batch_index):
    global one_files, two_files, three_files, unknown_files, numbers, four_files, five_files, six_files
    ...
    for i, filename in enumerate(files):
        ...
        #     num_button = 2
        # elif (filename.startswith("six")):
        #     num_button = 3

        print(f"[{device.port}] Sending sample {filename} ({i}/{len(files)}): Button {num_button}")
        sendSample(device, 'datasets_mine/train_json_one/'+filename, num_button, deviceIndex)
        # sendSample(device, 'datasets_mine/train_json_four/'+filename, num_button, deviceIndex)
...
def sendTestSamples(device, deviceIndex, successes_queue):
    global test_numbers, accuracy, test_one_files, test_two_files, test_three_files, test_unknown_files, test_four_files, test_five_files, test_six_files
    ...
    for i, filename in enumerate(files):
        ...
        # error, success = sendSample(device, 'datasets_mine/test_json_four/'+filename, num_button, deviceIndex, True)
...
def read_graph(device, deviceIndex):
    ...
    if (nb != b'5') and (not train_test):
        ...
        #     graph.append([n_epooch, n_error, deviceIndex])
        # else:
        #     graph_test.append([n_epooch, n_error, deviceIndex])
...
def plot_graph():
    ...
    if (repaint_graph):
        ...
        for device_index, device in enumerate(devices):
            ...
            # epoch_test = [x[0] for x in graph_test if x[2] == device_index]
            # error_test = [x[1] for x in graph_test if x[2] == device_index]
            # error_test = [x[0] for x in accuracy if x[1] == device_index]


            epochs = max(len(error), epochs)
            # print(f"Error: {error} with device {device_index}")
            # if train_test:
            #     # plt.subplot(211)
            #     ax1.plot(error, colors[device_index] + markers[device_index], label=f"Device {device_index}")
            #     # plt.subplot(212)
            #     ax2.plot(error_test, colors[device_index] + markers[device_index], label=f"Device {device_index}")
            if experiment != None:
                ...
                if (nb != b'5'):
                    ...
                    while True:
                        ...
                        # print(f"label: {type(label_predicted)}")
...
if experiment != None:
    ...
    np.save('hidden_one_two_three_3.npy', hidden_layer)
    np.save('output_one_two_three_3.npy', output_layer)
